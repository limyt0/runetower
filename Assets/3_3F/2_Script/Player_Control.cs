...
public class Player_Control : MonoBehaviour
{
    ...
    float jump_power = 2000f; //점프 정도
    RaycastHit objhit;

    jump_state js = jump_state.ground;
    private void Start()
    {
    ...
    void player_jump() {
        ...
        if (Input.GetKeyDown(KeyCode.Space) == true  && js == jump_state.ground)
        {
            print("jump");
    ...
    {
        if (js != jump_state.ground) js = jump_state.ground;
        print("지금상태는: "+ js);
